...
@section('breadcrumb', 'Manajer Gudang / Laporan / Stok')
...
<div class="space-y-5">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-4">
        <form method="GET" class="flex flex-wrap gap-3">
            ...
            <div class="flex items-end">
                <a href="{{ route('manager.reports.stock') }}" class="h-9 px-4 text-sm font-medium border border-gray-200 text-gray-500 rounded-xl hover:bg-gray-50 flex items-center">Reset</a>
            </div>
            @endif
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tr class="hover:bg-gray-50/50 transition">
                    ...
                    <td class="px-6 py-3.5 text-xs text-gray-600">{{ $p->category->name ?? '—' }}</td>
                    <td class="px-6 py-3.5 text-xs text-gray-600">{{ $p->supplier->name ?? '—' }}</td>
                    <td class="px-6 py-3.5 text-right font-bold {{ $p->stock == 0 ? 'text-red-600' : ($p->is_low_stock ? 'text-amber-600' : 'text-gray-800') }}">
                        {{ $p->stock }} <span class="text-xs font-normal text-gray-400">{{ $p->unit }}</span>
                    </td>
                    <td class="px-6 py-3.5 text-right text-gray-500">{{ $p->minimum_stock }}</td>
                    <td class="px-6 py-3.5 text-right font-medium text-gray-700">
                ...
                @empty
                <tr><td colspan="7" class="px-6 py-10 text-center text-gray-400">Tidak ada data.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
