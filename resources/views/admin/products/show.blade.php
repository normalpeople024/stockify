...
@section('breadcrumb', 'Admin / Produk / Detail')
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div class="xl:col-span-2 space-y-5">
                        ...
                        @if($product->is_low_stock)
                            <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-red-50 text-red-700 border border-red-200">
                                <svg class="w-3 h-3" fill="none" viewBox="0 0 16 16">
                                    <path d="M8 6v4M8 11.5v.5M6.29 2.57L1.14 11a2 2 0 001.71 3h10.3a2 2 0 001.71-3L9.71 2.57a2 2 0 00-3.42 0z" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/>
                                </svg>
                                Stok Menipis
                            </span>
                        @endif
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-6">
                ...
                <div class="mt-4 p-4 bg-blue-50 rounded-xl flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-blue-600 mt-0.5">{{ $product->stock }} × Rp {{ number_format($product->purchase_price, 0, ',', '.') }}</p>
                    </div>
                ...
                @php
                    ...
                    $marginPct = $product->purchase_price > 0
                        ? round(($margin / $product->purchase_price) * 100, 1)
                        : 0;
                @endphp
                <div class="mt-3 p-4 bg-amber-50 rounded-xl flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-amber-400 uppercase tracking-wider">Margin Keuntungan</p>
                        <p class="text-sm text-amber-600 mt-0.5">per {{ $product->unit }}</p>
                    </div>
                    <div class="text-right">
                        <p class="text-xl font-bold text-amber-700">
                            Rp {{ number_format($margin, 0, ',', '.') }}
                        </p>
                        <p class="text-xs font-semibold text-amber-500">{{ $marginPct }}%</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-50">
                    ...
                    <span class="text-xs text-gray-400">{{ $product->stockTransactions->count() }} transaksi</span>
                </div>
                ...
                <div class="px-6 py-10 text-center">
                    ...
                    <p class="text-xs text-gray-400 mt-1">Riwayat transaksi produk ini akan muncul di sini</p>
                </div>
                @endif
            </div>
        ...
        <div class="space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between py-2">
                        <span class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Terakhir Update</span>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-5 space-y-2">
                ...
                <a href="{{ route('admin.reports.transactions', ['product_id' => $product->id]) }}"
